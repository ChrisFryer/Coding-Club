...
$vCenterServer = "172.16.200.68"
$username = "script.test@vsphere.local"
...
try {
	...
	if ($snapshotInfo.Count -gt 0) {
        ...
        if ($oldSnapshots) {
			...
			$smtpServer = "172.16.230.70"  # Update with your Exchange server
            $smtpPort = 587
            $fromAddress = "noreply@healthcarecloud.com.au"  # Update with your sender address
            $toAddress = "chris.fryer@healthcarecloud.com.au"  # Update with recipient address(es) # Can be multiple recipients separated by commas
			# Email Creds via secure string
			$emailusername = "noreply"
